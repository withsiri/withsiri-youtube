import todayView01 from "../assets/img/today/todayView01.jpg"
import todayView02 from "../assets/img/today/todayView02.jpg"
import todayView03 from "../assets/img/today/todayView03.jpg"
import todayView04 from "../assets/img/today/todayView04.jpg"
import todayView05 from "../assets/img/today/todayView05.jpg"
import todayView06 from "../assets/img/today/todayView06.jpg"
import todayView07 from "../assets/img/today/todayView07.jpg"
import todayView08 from "../assets/img/today/todayView08.jpg"

export const todayText = [
    {
        img: todayView01,
        title: "나만의 유튜브 사이트 만들기",
        desc: "유투브 설명설명설명설명, 유튜브 설명설명설명 언제 어디서나 빠르고 간편하게 수업을 시작할 수 있도록 통합 계정과 브라우저로 수업에 필요한 도구들을 연결합니다. 유투브 설명설명설명설명, 유튜브 설명설명설명 언제 어디서나 빠르고 간편하게 수업을 시작할 수 있도록 통합 계정과 브라우저로 수업에 필요한 도구들을 연결합니다. 유투브 설명설명설명설명, 유튜브 설명설명설명 언제 어디서나 빠르고 간편하게 수업을 시작할 수 있도록 통합 계정과 브라우저로 수업에 필요한 도구들을 연결합니다.",
        author: "author",
        channelId: "",
        date: "2023.01.03",
        page: "/youtube"
    },
    {
        img: todayView02,
        title: "유투브02",
        desc: "",
        author: "",
        channelId: "",
        date: "2023.01.03",
        page: "/port"
    },
    {
        img: todayView03,
        title: "유투브03",
        desc: "",
        author: "",
        channelId: "",
        date: "2023.01.03",
        page: "/port"
    },
    {
        img: todayView04,
        title: "유투브04",
        desc: "",
        author: "",
        channelId: "",
        date: "2023.01.03",
        page: "/port"
    },
    {
        img: todayView05,
        title: "유투브05",
        desc: "",
        author: "",
        channelId: "",
        date: "2023.01.03"
    },
    {
        img: todayView06,
        title: "유투브06",
        desc: "",
        author: "",
        channelId: "",
        date: "2023.01.03",
        page: "/port"
    },
    {
        img: todayView07,
        title: "유투브07",
        desc: "",
        author: "",
        channelId: "",
        date: "2023.01.03",
        page: "/port"
    },
    {
        img: todayView08,
        title: "유투브08",
        desc: "",
        author: "",
        channelId: "",
        date: "2023.01.03",
        page: "/port"
    },
]